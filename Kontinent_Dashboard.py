...
# --- 2. PARSER UNIVERSEL (TEXTE + JSON) AVEC FIX POLLUTION ---
...
def universal_prosperity_parser(file_path):
    ...
    # --- FIX POLLUTION : Mid Price 0 ou NaN ---
    if not df_act.empty:
        df_act = df_act.sort_values(['product', 'timestamp'])
        # On remplace les 0 et NaN par NaN pour forcer le forward fill
        mask_bad = (df_act['mid_price'] <= 0) | (df_act['mid_price'].isna())
        df_act.loc[mask_bad, ['mid_price', 'profit_and_loss']] = np.nan
        
        # On remplit par la dernière valeur valide connue par produit
    ...
    if not df_tr.empty:
        ...
        if not df_act.empty:
            ...
            # Correction .ffill() après merge_asof
...
def main():
    ...
    with t_pnl:
        ...
        fig.update_layout(template="plotly_dark", height=500, title="Equity Curve (Cleaned Forward-Fill)")
    ...
    with t_heat:
        ...
        fig_h = px.imshow(heat_pivot, color_continuous_scale='RdYlGn', title="Profitability Heatmap")
    ...
    with t_vol:
        ...
        if not df_bot_f.empty:
            ...
            prod_select = st.selectbox("Market Profile Détail", options=selected)
